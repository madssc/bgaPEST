    ...
    do i=1,BL(1)%numkw
        if(BL(1)%keywordline(i) .ne. 0) then
            ...
            case ('jacobian_file')
                ...
                !Introduced for PCGA:
            case('PCGA')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%PCGA)
            case('non_reguar_grid')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%non_complete_grid)
            case('posterior_cov_compression_flag')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%posterior_cov_compression_flag)
            case('Nthread')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%Nthread)
            case('LM')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%LM)
            case('LM_lambda')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%LM_lambda)
            case('LM_down')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%LM_down)
            case('LM_up')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%LM_up)
            case('LM_min')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%LM_min)
            case('LM_max')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%LM_max)
            case('LM_max_it')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%LM_max_it)
            case('method_Gaussian')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%method_Gaussian)
            case('seed_Gaussian')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%seed_Gaussian)
            case('brng_Gaussian')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%brng_Gaussian)
            case('deriv_type')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%deriv_type)
            case('deriv_accuracy')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%deriv_accuracy)
            case('derivative_test')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%derivative_test)
            case('derivative_test_min')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%derivative_test_min)
            case('derivative_test_max')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%derivative_test_max)
            case('derivative_test_step')
                call drealread(ifail,BL(1)%keywordstring(i), cv_A%derivative_test_step)
            case('eigenvalue_test')
                call intread(ifail,BL(1)%keywordstring(i), cv_A%eigenvalue_test)
            end select
        endif
    enddo
    ...
    !Introduced for PCGA
    if (cv_A%PCGA /= 1) then !When not using PCGA
        cv_A%PCGA = 0
        cv_A%LM = 0
        cv_A%derivative_test = 0
        cv_A%non_complete_grid = 0
    endif
    if (cv_A%LM /= 1) cv_A%LM = 0 !When not using Levenberg-Marquardt
    if (cv_A%derivative_test /= 1 .and. cv_A%derivative_test /= 2 .and. cv_A%derivative_test /= 3) cv_A%derivative_test = 0 !When not using derivative test
    if (cv_A%derivative_test == 1 .or. cv_A%derivative_test == 2 .or. cv_A%derivative_test == 3)   cv_A%derivative_test_step = abs(cv_A%derivative_test_step)
    if (cv_A%non_complete_grid /= 1) cv_A%non_complete_grid = 0 !When not using non complete grid
    if (cv_A%Nthread < 0) then !Multithreaded matrix-free computations are not implemented yet
        write (*,*) "Nthread has to be zero or positive. Nthread is set to zero."
        cv_A%Nthread = 0
    endif
...
10      format('Error: The number of beta associations in Q_compression_cv block differs from', &
    ...
    subroutine bdp_read_parameter_groups(BL,cv_A,cv_PAR,inunit,retmsg)
    ...
    case(0)
        ...
        enddo
    case(1)
        allocate(cv_PAR%K(cv_PAR%npargp))
        allocate(cv_PAR%K_s(cv_PAR%npargp))
        cv_PAR%K = 0
        cv_PAR%K_s = 0
        numcol = 4
        allocate(columnstring(numcol))
        allocate(columnname(numcol))
        columnname=(/'groupname','grouptype','K','K_s'/) !Loading K and K_s as well. If K_s should be renamed it should be here.
        columnstring=(/' ',' ',' ',' '/)

        do i=1,cv_PAR%npargp
            call ids_read_block_table(ifail,bl(10)%label,4,columnname,columnstring,line,filename)
            cv_PAR%grp_name(i) = trim(adjustl(columnstring(1)))
            call intread(ifail, columnstring(2),cv_PAR%grp_type(i))
            call intread(ifail, columnstring(3),cv_PAR%K(i))
            call intread(ifail, columnstring(4),cv_PAR%K_s(i))
        enddo
    end select
...
500             format('Error: Beta association value ',i6, ' exceeds the number of beta values', i6, &
    ...
    !MC OBS: får alle den samme grp_type? I believe there is an error here. loop over j=1,cv_PAR%npar and i=1, cv_PAR%npargp!
...
10      format('Warning: No structural parameter optimization requested. it_max_bga in algorithmic_cv block must be 1.',& !| no structural
    ...
    if (cv_A%deriv_mode .eq. 4) then ! parameter groups used - (cv_A%deriv_mode .eq. 4)
        ...
        case(0)
            ...
            enddo
        case(1) !Mads
            allocate (d_MIO%derinc_PCGA(cv_MIO%ntplfle))
            allocate (columnname(3))
            allocate (columnstring(3))
            d_MIO%derinc_PCGA = UNINIT_REAL ! array
            ! first read input/tpl information
            columnname=(/'TemplateFile','ModInFile','Derinc'/)
            columnstring=' ' ! array
            do i=1,cv_MIO%ntplfle
                call ids_read_block_table(ifail,bl(15)%label,3,columnname,columnstring,line,filename)
                d_MIO%tpl(i) = trim(adjustl(columnstring(1)))
                d_MIO%infle(i) = trim(adjustl(columnstring(2)))
                call drealread(ifail, trim(adjustl(columnstring(3))) , d_MIO%derinc_PCGA(i) ) !Loading the delta parameter for the matrix-free approach
            enddo
            if (associated(columnname))           deallocate(columnname)
            allocate (columnname(2))
        end select
    ...
    subroutine bdp_read_data_observations(BL,d_OBS, cv_OBS, cv_A, inunit,retmsg,miostruc)
    use bayes_pest_control
    use jupiter_input_data_support
    use utilities
    use model_input_output
    ...
    case(1)
        numcol=5
        allocate (d_OBS%TemplateFileNo(cv_OBS%nobs))
        allocate (columnname(numcol))
        allocate (columnstring(numcol))
        d_OBS%TemplateFileNo = UNINIT_INT ! array

        columnname=(/'ObsName',  'ObsValue',  'GroupName',  'Weight', 'TemplateFileNo'/) !Introducing template file number for the matrix-free approach
        columnstring=' '  ! array
        ...
        do j=1,cv_OBS%nobs
            ...
            d_OBS%group(k)     =  trim(adjustl(columnstring(3)))
            call drealread(ifail, columnstring(4),d_OBS%weight(k) )
            call intread(ifail,   columnstring(5),d_OBS%TemplateFileNo(k) )
